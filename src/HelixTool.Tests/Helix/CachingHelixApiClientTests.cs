...
public class CachingHelixApiClientTests
{
    ...
    // =========================================================================
    // ListJobsByBuildAsync — no-cache forwarding
    // CachingHelixApiClient delegates directly to inner without caching.
    // =========================================================================

    [Fact]
    public async Task ListJobsByBuildAsync_ForwardsToInnerAndReturnsResult()
    {
        const string source = "ci/public/dotnet/runtime/refs/heads/main";
        const string buildId = "12345";
        var first = Substitute.For<IHelixJobSummary>();
        first.Name.Returns("job-guid-1");
        var second = Substitute.For<IHelixJobSummary>();
        second.Name.Returns("job-guid-2");
        IReadOnlyList<IHelixJobSummary> expected = [first, second];

        _inner.ListJobsByBuildAsync(source, buildId, 100_000, Arg.Any<CancellationToken>())
              .Returns(Task.FromResult(expected));

        var result = await _sut.ListJobsByBuildAsync(source, buildId);

        Assert.Equal(expected, result);
        await _inner.Received(1)
                    .ListJobsByBuildAsync(source, buildId, 100_000, Arg.Any<CancellationToken>());
    ...
    [Fact]
    public async Task ListJobsByBuildAsync_NoCache_InnerCalledTwiceOnTwoCalls()
    {
        ...
        summary.Name.Returns("job-guid-1");
        IReadOnlyList<IHelixJobSummary> jobs = [summary];

        _inner.ListJobsByBuildAsync(source, buildId, Arg.Any<int>(), Arg.Any<CancellationToken>())
              .Returns(Task.FromResult(jobs));

        await _sut.ListJobsByBuildAsync(source, buildId);
        await _sut.ListJobsByBuildAsync(source, buildId);

        await _inner.Received(2)
                    .ListJobsByBuildAsync(source, buildId, Arg.Any<int>(), Arg.Any<CancellationToken>());
    }
}
