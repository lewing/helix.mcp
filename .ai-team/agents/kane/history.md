...
📌 Team update (2026-02-27): MCP descriptions describe what/inputs/outputs, not implementation details — decided by Dallas. This resolves the P1 question about whether to update MCP descriptions with local-enhancement flags. Answer: no. The `hlx_status` description should mention `failureCategory` as a completeness fix (output field, not implementation detail). README enhancements section covers the "how" for human readers.

- **2025-07-18-llmstxt-missing-tools:** Fixed llmstxt MCP Tools list in Program.cs (lines 516-528). Three changes: (1) Added `hlx_search_file` — "Search a work item's uploaded file for lines matching a pattern" (from HelixMcpTools.cs:256 [Description]). (2) Added `hlx_test_results` — "Parse TRX test result files from a work item" (concise version of HelixMcpTools.cs:303 [Description]). (3) Updated `hlx_status` description to include `failureCategory` — now reads "Job pass/fail summary with consoleLogUrl and failureCategory per work item", matching the Failure Categorization section already present in llmstxt (line 538). This closes the P1 doc gap identified in the 2025-07-18 value-add audit.
