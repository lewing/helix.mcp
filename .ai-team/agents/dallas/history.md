...
- **Multi-auth analysis (2026-02-12):** Larry asked about supporting multiple simultaneous auth tokens. After full architectural review, decided against it. Key insight: the MCP stdio model already supports multi-auth via multiple server entries in the client config (each with its own env vars, process, DI container, and cache context). The CLI case is solved by the shell environment. Adding named connections, config files, or per-request auth would add complexity without user value. Cache isolation (`CacheOptions.AuthTokenHash` → SHA256 → separate SQLite DBs) already works correctly across auth contexts. MCP protocol is session-scoped auth, not per-tool-call — multi-auth within a single session would require non-standard extensions. See `.ai-team/decisions/inbox/dallas-multi-auth-analysis.md`.
...
- **HTTP/SSE multi-client auth architecture (2026-02-12):** Designed per-client token isolation for `HelixTool.Mcp` HTTP transport. Key findings from MCP C# SDK (`modelcontextprotocol/csharp-sdk`): (1) `HttpServerTransportOptions.ConfigureSessionOptions` callback receives `HttpContext` per new session. (2) `McpServerOptions.ScopeRequests = true` (default) creates DI scope per tool invocation. (3) `IHttpContextAccessor` works when `PerSessionExecutionContext = false` (default). (4) SDK enforces session-user binding via `ClaimsPrincipal`. Architecture decision: use `IHttpContextAccessor` + scoped `IHelixApiClient` — standard ASP.NET Core pattern. New abstractions: `IHelixTokenAccessor` (env var for stdio, `HttpContext.Authorization` for HTTP), `IHelixApiClientFactory`, `ICacheStoreFactory` (token-hash-keyed singleton cache stores). DI changes: `IHelixApiClient` and `HelixService` become Scoped in HTTP, stay Singleton in stdio. `IHelixApiClient` interface unchanged. Priority P2 — no current HTTP consumers. 10 Ripley + 4 Lambert action items defined. Risk: `SqliteCacheStore` single-connection design needs concurrent-access hardening for HTTP load. See `.ai-team/decisions/inbox/dallas-http-sse-auth.md`.

- **Key SDK types discovered:** `HttpServerTransportOptions` has `ConfigureSessionOptions`, `RunSessionHandler`, `PerSessionExecutionContext`, `Stateless`, `IdleTimeout`, `MaxIdleSessionCount`, `EventStreamStore`. `StreamableHttpHandler.CreateSessionAsync()` calls `McpServer.Create(transport, options, loggerFactory, services)` — the `services` parameter is `applicationServices` for stateful, `context.RequestServices` for stateless. `ReadJsonRpcMessageAsync` flows `context.User` into `JsonRpcMessage.Context.User`.

- **`HelixTool.Mcp/Program.cs` current state:** Minimal — 20 lines, singleton DI, `WithHttpTransport()` + `MapMcp()`. No cache, no auth. Uses `ModelContextProtocol.AspNetCore` v0.8.0-preview.1. Needs full rewrite for multi-client support per the architecture decision.
